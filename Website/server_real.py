...
class ServerCmd(cmd.Cmd):
    ...
    prompt = '(server) '
    # A new dictionary to keep track of the connected clients' details
    connected_clients = {}
    ...
    #         clients_info = [{"address": addr[0], "port": addr[1]} for addr in self.clients]
    #         socketio.emit('update_clients', clients_info, broadcast=True)
        ...
        while not self.shutdown_flag.is_set():  # Continue to listen unless a shutdown is signaled
            try:
                ...
                with self.clients_lock:
                    ...
                    # socketio.emit('new_client', client_addr)
                # self.update_connected_clients()  # Update the connected clients list
    ...
    def handle_client(self, client_conn, client_addr):
        ...
        with self.clients_lock:
            ...
            # socketio.emit('client_disconnected', client_addr)
        # self.update_connected_clients()  # Update the connected clients list when a client disconnects
        print(f"Client {client_addr} has disconnected")
        client_conn.close()
    # def get_connected_clients(self):
    #     with self.clients_lock:
    #         return [{"address": str(addr[0]), "port": addr[1]} for addr in self.clients.keys()]
...
def scan_server_on_machine(ip, port):
